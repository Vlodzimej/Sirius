...
export class InvoiceListComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.route.params.subscribe(params => {
            ...
            this.apiService
                .getById<InvoiceType>('invoice/type/alias', this.typeAlias)
                .subscribe(
                    data => {
                        this.type = data;
    ...
    onDelete(invoiceId: string) {
        this.apiService.delete('invoice', invoiceId).subscribe(
            data => {
                var deletedInvoice = this.invoices.find(
                    i => i.id == invoiceId
                ) as Invoice;
                console.log(deletedInvoice);
                if (deletedInvoice.isFixed) {
                    this.alertService.error(
                        'Документ проведён. Удаление невозможно.'
                    );
                } else {
    ...
    getInvoicesByFilter() {
        ...
        params +=
            filter.startDate != null ? '&startDate=' + filter.startDate : '';
        params +=
            filter.finishDate != null ? '&finishDate=' + filter.finishDate : '';
        params +=
            filter.fixedOnly == true ? '&fixedOnly=true' : '&fixedOnly=false';
